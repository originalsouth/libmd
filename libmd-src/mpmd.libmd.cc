...
template<ui dim> ldf mpmd<dim>::embedded_dd_p1(ui d,ldf x1[dim],ldf x2[dim])
{
    duals<dim> z1,z2;
    patch.calc(z1,x1);
    patch.calc(z2,x2);
...
template<ui dim> ldf mpmd<dim>::embedded_dd_p2(ui d,ldf x1[dim],ldf x2[dim])
{
    duals<dim> z1,z2;
    patch.calc(z1,x1);
    patch.calc(z2,x2);
    return dd(d,x2,x1)+((z1.x-z2.x)*z2.dx[d]);
}
