...
struct var
{
    ldf val, x;
	var() {}
    var (ldf f, ldf fx = 1.0) : val(f), x(fx) {}
	ldf diff() { return x; }
};

var operator+ (const var& F, const var& G)
{
    return var(F.val+G.val, F.x+G.x);
}
...
{
    return var(-F.val, -F.x);
}

var operator* (const var& F, const var& G)
{
    return var(F.val*G.val, F.x*G.val + F.val*G.x);
}

var operator/ (const var& F, const var& G)
{
    return var(F.val/G.val, (F.x*G.val - F.val*G.x) / pow(G.val,2));
}

template<class number> var operator+ (const var& F, number a)
{
    return var(F.val+a, F.x);
}
...
{
    return var(sqrt(F.val), F.x / 2.0 / sqrt(F.val));
}
...
var tanh (const var& F)
{
    return sinh(F)/cosh(F);
}

var asin (const var& F)
{
    return var(asin(F.val),F.x/sqrt(1.0-pow(F.val,2)));
}

var acos (const var& F)
{
    return var(acos(F.val),-F.x/sqrt(1.0-pow(F.val,2)));
}

var atan (const var& F)
{
    return var(atan(F.val),F.x/(pow(F.val,2)+1.0));
}
...
struct var2
{
    ldf val, x, y, xy;
	var2() {}
    var2 (ldf f, ldf fx = 1.0, ldf fy = 1.0, ldf fxy = 0.0) : val(f), x(fx), y(fy), xy(fxy) {}
	ldf diff1() { return x; }
	ldf diff2() { return y; }
